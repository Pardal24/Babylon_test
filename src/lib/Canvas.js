...
const Canvas = ({
  ...
  const onDrag = useCallback((position, area) => {
    ...
    const centerX = x + pointSize + 5; // Calculate the center x-coordinate
    const centerY = y + pointSize -95; // Calculate the center y-coordinate
    ...
    magnCtx.drawImage(
      ...
      // Draw the point in the middle of the magnifier
      magnCtx.fillStyle = 'red'; // Set the fill color for the point
      magnCtx.beginPath();
      magnCtx.arc(centerX, centerY, 5, 0, Math.PI * 2); // Draw a circle
      magnCtx.fill();
